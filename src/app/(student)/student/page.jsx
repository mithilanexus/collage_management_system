...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const savedCollapsed = localStorage.getItem('studentDashboard_collapsed');
    const savedHidden = localStorage.getItem('studentDashboard_hidden');
  ...
  const toggleSection = (sectionId) => {
    ...
    localStorage.setItem('studentDashboard_collapsed', JSON.stringify(newCollapsed));
  };

  const toggleCardVisibility = (cardId) => {
    const newHidden = {
      ...hiddenCards,
      [cardId]: !hiddenCards[cardId]
    };
    setHiddenCards(newHidden);
    localStorage.setItem('studentDashboard_hidden', JSON.stringify(newHidden));
  };
  const quickStats = [
    {
      title: "Current GPA",
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                variant="ghost"
                size="sm"
                onClick={() => toggleCardVisibility('quickStats')}
                className="p-1"
              >
                {hiddenCards.quickStats ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
              </Button>
              <Button
                variant="ghost"
                size="sm"
                ...
                className="p-1"
              >
                {collapsedSections.quickStats ? <ChevronDown className="w-4 h-4" /> : <ChevronUp className="w-4 h-4" />}
              </Button>
            </div>
          </div>
        </CardHeader>
        {!collapsedSections.quickStats && !hiddenCards.quickStats && (
          <CardContent>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => toggleCardVisibility('currentCourses')}
                    className="p-1"
                  >
                    {hiddenCards.currentCourses ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                  </Button>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => toggleSection('currentCourses')}
                    className="p-1"
                  >
                    {collapsedSections.currentCourses ? <ChevronDown className="w-4 h-4" /> : <ChevronUp className="w-4 h-4" />}
                  </Button>
                </div>
              </div>
            </CardHeader>
            {!collapsedSections.currentCourses && !hiddenCards.currentCourses && (
              <CardContent>
        ...
        <div className="space-y-6">
          ...
          <Card>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => toggleCardVisibility('notifications')}
                    className="p-1"
                  >
                    {hiddenCards.notifications ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                  </Button>
                  <Button
                    variant="ghost"
                    size="sm"
            ...
            {!collapsedSections.notifications && !hiddenCards.notifications && (
              <CardContent>
